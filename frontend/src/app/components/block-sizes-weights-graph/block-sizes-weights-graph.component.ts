...
export class BlockSizesWeightsGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.bitcoin.graphs.block-sizes:See Monero block sizes visualized over time.`);
        ...
        formatter: (ticks) => {
          ...
          for (const tick of ticks) {
            if (tick.seriesIndex === 0) { // Size
              ...
              tooltip += `${tick.marker} ${tick.seriesName}: ${formatNumber(tick.data[1], this.locale, '1.2-2')} B/B`;
            }
            tooltip += `<br>`;
          }
